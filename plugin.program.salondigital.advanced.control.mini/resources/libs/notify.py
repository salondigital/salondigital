	...
	class MyWindow(xbmcgui.WindowDialog):
		...
		def doWrite(self):
			...
			if os.path.exists(ADVANCED):
				choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Hay un [COLOR %s]AdvancedSettings.xml[/COLOR] activo, quieres eliminarlo y continuar??[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLOR green]Eliminar[/COLOR][/B]", nolabel="[B][COLOR red]Cancelar[/COLOR][/B]")
				if choice == 0: return
...
def apkInstaller(apk):
	...
	xbmc.executebuiltin('Skin.SetString(apkinstaller, Ahora que %s se ha descargado[CR]Pulsa el boton principal para instalar!)' % apk)
...
def firstRun():
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			...
			self.msg   = "No tienes instalada ninguna version de %s.\n\nSelecciona 'Instalar' para instalar una version o actualizacion o 'Ignorar' para no volver a ver este mensaje.\n\nGracias por confiar en %s." % (ADDONTITLE, ADDONTITLE)
...
def updateWindow(name='Testing Window', current='1.0', new='1.1', icon=ICON, fanart=FANART):
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			...
			self.fanart = kwargs['fanart']
			self.msgupdate  = "Nueva actualizacion disponible para:\n[COLOR %s]%s[/COLOR]\n\nVersion instalada: v[COLOR %s]%s[/COLOR]\nUltima Version: v[COLOR %s]%s[/COLOR]\n\n[COLOR %s]*Recomendacion: Instalacion limpia[/COLOR]" % (COLOR1, self.name, COLOR1, self.current, COLOR1, self.new, COLOR1)
			self.msgcurrent = "Tienes instalada la ultima version disponible de:\n[COLOR %s]%s[/COLOR]\n\nVersion instalada: v[COLOR %s]%s[/COLOR]\nUltima Version: v[COLOR %s]%s[/COLOR]\n\n[COLOR %s]*Recomendacion: Instalacion limpia[/COLOR]" % (COLOR1, self.name, COLOR1, self.current, COLOR1, self.new, COLOR1)
