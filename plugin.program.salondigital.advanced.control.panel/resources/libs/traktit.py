...
def importlist(who):
	if who == 'all':
		...
		if os.path.exists(TRAKTID[who]['file']):
			...
			if len(m) > 0:
				if not m[0] == su:
					if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres importar los datos de [COLOR %s]Trakt[/COLOR] para [COLOR %s]%s[/COLOR]?" % (COLOR2, COLOR1, COLOR1, n), "File: [COLOR green][B]%s[/B][/COLOR]" % m[0], "Guardado:[/COLOR] [COLOR red][B]%s[/B][/COLOR]" % su if not su == '' else 'Guardado:[/COLOR] [COLOR red][B]Ninguno[/B][/COLOR]', yeslabel="[B]Guardar datos[/B]", nolabel="[B]No, Cancelar[/B]"):
						wiz.setS(sa, m[0])
