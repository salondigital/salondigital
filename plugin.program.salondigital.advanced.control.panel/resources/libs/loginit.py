...
def clearSaved(who, over=False):
	if who == 'all':
		...
		if os.path.exists(file): 
			os.remove(file)
			wiz.LogNotify('[COLOR %s]%s[/COLOR]' % (COLOR1, LOGINID[who]['name']), '[COLOR %s]Login Data: Eliminado![/COLOR]' % COLOR2, 2000, LOGINID[who]['icon'])
