...
class Main:
	def __init__(self):
		...
		for item in files:
			...
			if succes:
				...
				if succes:
					msg = "Escribe esta URL para el codigo QR de [COLOR %s]%s[/COLOR], con una descripcion del problema:[CR][COLOR %s]%s[/COLOR]" % (COLOR1, name, COLOR1, result)
					if len(self.email) > 5: 
	...
	def email_Log(self, email, results, file):
		...
		try:
			...
			a = 'Fallo al conectar al servidor'
			wiz.log("%s: %s" % (a, str(e)), xbmc.LOGERROR)
			return False, a
		try:
